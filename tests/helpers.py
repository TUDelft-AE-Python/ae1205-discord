"""Contains Unit-Testing Mock Classes for testing the Discord bot.

This file has taken directly from the lovely `Python Discord`_ bot
:py:module:`tests.helpers` module. All rights remain with the original
authors!

`Python Discord`_ https://github.com/python-discord/bot
"""
...
for logger in logging.Logger.manager.loggerDict.values():
    # Set all loggers to CRITICAL by default to prevent screen clutter
    # during testing
...
async def async_magic():
    """Creating a dummy asynchronious method for monkey-patching."""
    pass


# Monkey patching MagicMock to be useable with asyncio
# See: https://stackoverflow.com/a/51399767/11989587
unittest.mock.MagicMock.__await__ = lambda x: async_magic().__await__()


class HashableMixin(discord.mixins.EqualityComparable):
    """Provides hashing and equality functionality.

    Note:
        This is simlar to discord.py's `Hashable` mixin, however
        discord.py`s `Hashable` mixin bit-shifts `self.id` (`>> 22`); to
        prevent hash-collisions for the relative small `id` integers we
        generally use in tests, this bit-shift is omitted.
    """
...
class ColourMixin:
    ...
    @property
    def color(self) -> discord.Colour:  # noqa
        return self.colour
...
class CustomMockMixin:
    ...
    The `_get_child_mock` method automatically returns an AsyncMock for
    coroutine methods of the mock object. As discord.py also uses
    synchronous methods that nonetheless return coroutine objects, the
    class attribute `additional_spec_asyncs` can be overwritten with an
    iterable containing additional attribute names that should also
    mocked with an AsyncMock instead of a regular MagicMock/Mock. The
    class method `spec_set` can be overwritten with the object that
    should be uses as the specification for the mock.

    Mock/MagicMock subclasses that use this mixin only need to define
    `__init__` method if they need to implement custom behavior.
    """
    ...
    def __init__(self, **kwargs):
        name = kwargs.pop(
            "name", None
        )  # `name` has special meaning for Mock classes, setting manually.
    ...
    def _get_child_mock(self, **kw):
        """Overwrites `_get_child_mock` stopping propagation of mock.

        Mock objects automatically create children when you access an
        attribute or call a method on them. By default, the class of
        these children is the type of the parent itself. However, this
        would mean that the children created for our custom mock types
        would also be instances of that custom mock type. This is not
        desirable, as attributes of, e.g., a `Bot` object are not `Bot`
        objects themselves. The Python docs for `unittest.mock` hint
        that overwriting this method is the best way to deal with that.

        This override will look for an attribute called
        `child_mock_type` and use that as the type of the child mock.
        """
...
class MockGuild(CustomMockMixin, unittest.mock.Mock, HashableMixin):
    ...
    A MockGuild instance will follow the specifications of a
    `discord.Guild` instance. This means that if the code you're testing
    tries to access an attribute or method that normally does not exist
    for a `discord.Guild` object this will raise an `AttributeError`.
    This is to make sure our tests fail if the code we're testing uses a
    `discord.Guild` object in the wrong way.

    One restriction of that is that if the code tries to access an
    attribute that normally does not exist for `discord.Guild` instance
    but was added dynamically, this will raise an exception with the
    mocked object. To get around that, you can set the non-standard
    attribute explicitly for the instance of `MockGuild`::

        >>> guild = MockGuild()
        >>> guild.attribute_that_normally_does_not_exist = (
        ...     unittest.mock.MagicMock()
        ... )

    In addition to attribute simulation, mocked guild object will pass
    an `isinstance` check against `discord.Guild`::
    ...
    def __init__(
        self, roles: Optional[Iterable[MockRole]] = None, **kwargs
    ) -> None:
...
class MockRole(
    CustomMockMixin, unittest.mock.Mock, ColourMixin, HashableMixin
):
    """A Mock subclass to mock `discord.Role` objects.

    Instances of this class will follow the specifications of
    `discord.Role` instances. For more information, see the `MockGuild`
    docstring.
    """
    ...
    def __lt__(self, other):
        ...
        Note:
            This is similar to those of `discord.Role`.
        """
...
class MockMember(
    ...
    Instances of this class will follow the specifications of
    `discord.Member` instances. For more information, see the
    `MockGuild` docstring.
    """
...
class MockUser(
    ...
    Instances of this class will follow the specifications of
    `discord.User` instances. For more information, see the `MockGuild`
    docstring.
    """
...
# Create a Bot instance to get a realistic MagicMock of
...
class MockBot(CustomMockMixin, unittest.mock.MagicMock):
    ...
    Instances of this class will follow the specifications of
    `discord.ext.commands.Bot` instances. For more information, see the
    `MockGuild` docstring.
    """
    ...
    def __init__(self, **kwargs) -> None:
        ...
        # this `side_effect` calls `close()` on the coroutine object to
        # prevent "has not been awaited"-warnings.
        self.loop.create_task.side_effect = lambda coroutine: coroutine.close()


# Create a TextChannel instance to get a realistic MagicMock of
# `discord.TextChannel`
channel_data = {
    "id": 1,
    "type": "TextChannel",
...
class MockTextChannel(CustomMockMixin, unittest.mock.Mock, HashableMixin):
    ...
    Instances of this class will follow the specifications of
    `discord.TextChannel` instances. For more information, see the
    `MockGuild` docstring.
    """
    ...
    def __init__(
        self, name: str = "channel", channel_id: int = 1, **kwargs
    ) -> None:
        default_kwargs = {
            "id": next(self.discord_id),
            "name": "channel",
...
# Create a Message instance to get a realistic MagicMock of
# `discord.Message`
message_data = {
    "id": 1,
...
# Create a Context instance to get a realistic MagicMock of
...
class MockContext(CustomMockMixin, unittest.mock.MagicMock):
    ...
    Instances of this class will follow the specifications of
    `discord.ext.commands.Context` instances. For more information, see
    the `MockGuild` docstring.
    """
...
class MockAttachment(CustomMockMixin, unittest.mock.MagicMock):
    ...
    Instances of this class will follow the specifications of
    `discord.Attachment` instances. For more information, see the
    `MockGuild` docstring.
    """
...
class MockMessage(CustomMockMixin, unittest.mock.MagicMock):
    ...
    Instances of this class will follow the specifications of
    `discord.Message` instances. For more information, see the
    `MockGuild` docstring.
    """
...
class MockEmoji(CustomMockMixin, unittest.mock.MagicMock):
    ...
    Instances of this class will follow the specifications of
    `discord.Emoji` instances. For more information, see the `MockGuild`
    docstring.
    """
...
class MockPartialEmoji(CustomMockMixin, unittest.mock.MagicMock):
    ...
    Instances of this class will follow the specifications of
    `discord.PartialEmoji` instances. For more information, see the
    `MockGuild` docstring.
    """
...
class MockReaction(CustomMockMixin, unittest.mock.MagicMock):
    ...
    Instances of this class will follow the specifications of
    `discord.Reaction` instances. For more information, see the
    `MockGuild` docstring.
    """
...
webhook_instance = discord.Webhook(
    data=unittest.mock.MagicMock(), adapter=unittest.mock.MagicMock()
)


class MockAsyncWebhook(CustomMockMixin, unittest.mock.MagicMock):
    """A MagicMock subclass to mock Webhook objects.

    Instances of this class will follow the specifications of
    `discord.Webhook` instances. For more information, see the
    `MockGuild` docstring.

    Note:
        This is using an AsyncWebhookAdapter.
    """
